...
<nav class="top-right">
	<a class="disc l1"  href="${pageContext.request.contextPath}/requestout.action">
		<div>注销</div>
	</a>
	<a  class="disc l2" href="${pageContext.request.contextPath}/index.jsp">
		<div>主页</div>
	</a>
	<a class="disc l3" href="${pageContext.request.contextPath}/help.jsp">
		<div>帮助</div>
	</a>
	<a class="disc l4" href="${pageContext.request.contextPath}/shutdown.action">
		<div>关闭服务器</div>
	</a>
...
<script src="js/nav.js"></script>
  ...
  <hr color="blue" size="2"/><br/>
   ...
   <form action="${pageContext.request.contextPath}/upload.action" method="post" enctype="multipart/form-data">
    	...
    	<input class="btn btn-outline-primary"type="file" onchange="checkfile()" id="fileupload" name="file" onpropertychange="getFileSize(this.value)"/><br/>
   ...
   <br/>
      ...
      <th scope="col">操作</th>
    	...
    	<c:forEach var="c" items="${requestScope.pagebean.list}" varStatus="stat">
    		<tr >
	    		...
	    		<td>${c.createtime }</td>
	    		<td>
	    			<a class="btn btn-success" href="${pageContext.request.contextPath}/download.action?id=${c.id }&filename=${c.filename }">下载</a>
	    		</td>
	    		<td>
	    		<form>
    ...
    <p class="text-secondary">
     ...
     每页 <input type="text" id = "pagesize" value="${pagebean.pagesize }" onchange="gotopage(${pagebean.currentpage})" style="10px" maxlength="5">条
      ...
      <a  class="btn btn-outline-primary" href="javascript:void(0)" onclick="gotopage(${requestScope.pagebean.previouspage})">上一页</a> 
      ...
      <input class="btn btn-outline-warning" type="button" value="跳转至第" onclick="gotopage1(document.getElementById('pagenum').value)" />
